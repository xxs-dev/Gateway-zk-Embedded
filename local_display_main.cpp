...
private:
    ...
    void appendWidgetIndexes(
        ...
        if (widget.type == "pointTable" || widget.type == "alarmSummary" || widget.type == "emsDashboard") {
    ...
    void appendWidgetConfigJson(std::ostringstream& out, const edge_gateway::LocalDisplayWidgetConfig& widget) const {
        ...
        out << ",\"progress\":{\"maxValue\":" << widget.progressMaxValue << "}"
            << ",\"position\":{"
            << "\"x\":" << widget.grid.col
            << ",\"y\":" << widget.grid.row
...
.ld-table-widget{padding:12px}.ld-table-wrap{height:calc(100% - 28px);overflow:auto}table{width:100%;border-collapse:collapse}th,td{padding:8px 10px;border-bottom:1px solid var(--line);white-space:nowrap;text-align:left}th{color:var(--muted);font-weight:500;background:#151d25}tr.bad td{color:var(--bad)}tr.stale td{color:var(--warn)}
.ld-battery-widget{padding:12px;display:grid;grid-template-rows:auto 1fr auto;gap:8px;background:rgba(24,33,41,.72)}.ld-battery-cap{width:54px;height:12px;margin:0 auto -2px;border:2px solid #64b8b8;border-bottom:0;border-radius:5px 5px 0 0;background:rgba(100,184,184,.45)}.ld-battery-body{position:relative;overflow:hidden;min-height:80px;border:4px solid #64b8b8;border-radius:14px;background:#315d5a}.ld-battery-fill{position:absolute;left:0;right:0;bottom:0;background:#2f8f85}.ld-battery-body:after{content:"";position:absolute;inset:5px;border:1px solid rgba(255,255,255,.16);border-radius:9px}.ld-battery-percent{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:700;color:#fff;text-shadow:0 2px 5px rgba(0,0,0,.35)}.ld-battery-label{text-align:center;color:var(--muted);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis})TEMPLATE";
    }

    std::string emsDashboardCss() const {
        return R"TEMPLATE(:root{--bg:#101820;--panel:#182129;--panel2:#141c24;--line:#26323d;--text:#e8f0f2;--muted:#8fa4ad;--ok:#16c784;--bad:#ff5c5c;--warn:#f5a623;--accent:#56b6ff}
*{box-sizing:border-box}body{margin:0;background:#101820;color:var(--text);font-family:"Microsoft YaHei","Noto Sans SC","Segoe UI",sans-serif}
.ems-shell{min-height:100vh;padding:18px 22px;display:grid;grid-template-rows:auto auto 1fr;gap:12px}.ems-header{display:grid;grid-template-columns:1fr minmax(280px,420px);gap:14px;align-items:end}.ems-title{font-size:30px;font-weight:700;line-height:1.1}.ems-sub,.ems-clock{color:var(--muted);font-size:14px;margin-top:8px}.ems-clock{text-align:right}.ems-search{width:100%;background:#0f171e;border:1px solid var(--line);border-radius:6px;color:var(--text);font-size:16px;padding:9px 11px}
.ems-cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.ems-card{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:13px 16px}.ems-card-title{color:var(--muted);font-size:14px}.ems-card-value{font-size:28px;font-weight:700;margin-top:7px}.ems-card-value.ok{color:var(--ok)}.ems-card-value.bad{color:var(--bad)}.ems-card-value.accent{color:var(--accent)}
.ems-tabs{min-height:0;display:grid;grid-template-rows:auto 1fr;border:1px solid var(--line);background:var(--panel)}.ems-tabbar{display:flex;gap:0;background:#101820;border-bottom:1px solid var(--line)}.ems-tab{min-width:120px;border:0;border-right:1px solid var(--line);background:#151d25;color:var(--muted);padding:11px 18px;font-size:15px;cursor:pointer}.ems-tab.active{background:#24465c;color:var(--text);font-weight:700}.ems-table-wrap{min-height:0;overflow:auto}.ems-table{width:100%;border-collapse:collapse;font-size:15px}.ems-table th{position:sticky;top:0;z-index:1;background:#151d25;color:var(--muted);font-weight:500}.ems-table th,.ems-table td{border:1px solid var(--line);padding:8px 10px;white-space:nowrap;text-align:left}.ems-table tbody tr:nth-child(even){background:var(--panel2)}.ems-table td.name{min-width:240px}.ems-table tr.bad td,.ems-table tr.bad td.value{color:var(--bad)}.ems-table tr.stale td,.ems-table tr.stale td.value{color:var(--warn)}.ems-table td.value.ok{color:var(--ok)}.ems-table td.muted{color:var(--muted)}
@media(max-width:900px){.ems-shell{padding:12px}.ems-header{grid-template-columns:1fr}.ems-clock{text-align:left}.ems-cards{grid-template-columns:repeat(2,minmax(0,1fr))}.ems-tab{min-width:auto;flex:1;padding:10px 8px}.ems-table{font-size:13px}})TEMPLATE";
    }
...
<body>
  ...
  <script>
    ...
    const emsDashboardCss = ")HTML" << escapeScriptString(emsDashboardCss()) << R"HTML(";
    const displayScreens = )HTML" << screensJsonArray() << R"HTML(;
    const templateRefreshIntervalMs = )HTML" << templateRefreshMs << R"HTML(;
    const displayRefreshIntervalMs = )HTML" << config_.refreshIntervalMs << R"HTML(;
    ...
    function pointMatches(p,f){return !f||String(p.index).includes(f)||(p.deviceName||"").toLowerCase().includes(f)||(p.meterCode||"").toLowerCase().includes(f)||(p.pointCode||"").toLowerCase().includes(f)||(p.name||"").toLowerCase().includes(f)||(p.category||"").toLowerCase().includes(f)}
    function filteredPoints(){const f=state.filter.toLowerCase();const pts=state.frame.points||[];return pts.filter(p=>pointMatches(p,f))}
    function pointField(p,field){const s=status(p);switch(field||"value"){case"index":return p?.index??"";case"deviceName":return p?.deviceName||"";case"meterCode":return p?.meterCode||"";case"pointCode":return p?.pointCode||"";case"name":return p?.name||"";case"category":return p?.category||"";case"unit":return p?.unit||"";case"value":return p?.hasValue?fmtVal(p.value):"-";case"rawValue":return p?.hasValue?p.value:"";case"quality":return p?.quality??"";case"time":return fmtTs(p?.valueTs);case"timestamp":return p?.valueTs||"";case"stale":return p?.stale?"true":"false";case"hasValue":return p?.hasValue?"true":"false";case"writable":return p?.writable?"是":"否";case"sharedMemoryName":return p?.sharedMemoryName||"";case"statusClass":return s.className;case"statusText":return s.text;case"statusColor":return s.color;default:return p&&Object.prototype.hasOwnProperty.call(p,field)?p[field]:""}}
    ...
    function labelOf(c){return {index:"Index",deviceName:"设备",meterCode:"仪表",pointCode:"点位",name:"名称",category:"分类",value:"值",unit:"单位",quality:"质量",time:"时间",statusText:"状态",writable:"写"}[c]||c}
    ...
    function batteryPercent(w,p){const max=Math.max(1,Number(w?.progress?.maxValue||100));const value=p&&p.hasValue?Number(p.value):0;return Math.max(0,Math.min(100,value/max*100))}
    ...
    function renderBatterySoc(w,st){const p=pointByIndex(w?.bind?.index);const s=status(p);const percent=batteryPercent(w,p);return `<section class="ld-widget ld-battery-widget ${esc(s.className)}" style="${st}"><div class="ld-widget-title">${esc(w.title||"电池SOC")}</div><div class="ld-battery-cap"></div><div class="ld-battery-body"><div class="ld-battery-fill" style="height:${percent.toFixed(1)}%"></div><div class="ld-battery-percent">${esc(percent.toFixed(0))}%</div></div><div class="ld-battery-label">${esc(p.name||p.pointCode||p.index||"未绑定")} ${esc(s.text)}</div></section>`}
    function renderWidget(w){const st=gridStyle(w);if(w?.type==="groupTitle")return `<section class="ld-widget ld-title" style="${st}"><div>${esc(w.text||w.title||"")}</div><span data-ld-clock>${esc(new Date().toLocaleString())}</span></section>`;if(w?.type==="pointTable"||w?.type==="alarmSummary")return `<section class="ld-widget ld-table-widget" style="${st}"><div class="ld-widget-title">${esc(w.title||"")}</div>${renderTable(w)}</section>`;if(w?.type==="batterySoc")return renderBatterySoc(w,st);const p=pointByIndex(w?.bind?.index);const s=status(p);return `<section class="ld-widget ld-card ${esc(s.className)}" style="${st}"><div class="ld-widget-title">${esc(w.title||"")}</div><div class="ld-value">${esc(widgetValue(w,p))}</div><div class="ld-meta">${esc(p.name||p.pointCode||p.index||"")} ${esc(p.unit||"")}</div><div class="ld-status">${esc(s.text)} / Q=${esc(p.quality??"")} / ${esc(fmtTs(p.valueTs))}</div></section>`}
    function emsPageKind(){return state.emsPage||"overview"}
    function emsKindMatches(kind,p){if(kind==="all"||kind==="run"||kind==="overview")return true;const t=`${p?.deviceName||""} ${p?.meterCode||""} ${p?.pointCode||""} ${p?.name||""} ${p?.category||""}`.toLowerCase();const idx=Number(p?.index||0);if(kind==="pcs")return t.includes("pcs")||(idx>=600&&idx<=650)||(idx>=1200&&idx<=1399);if(kind==="bms")return t.includes("bms")||t.includes("bat")||t.includes("soc")||t.includes("soh")||(idx>=1400&&idx<=3999);return true}
    function emsVisiblePoints(kind){const f=state.filter.toLowerCase();return (state.frame.points||[]).filter(p=>emsKindMatches(kind,p)&&pointMatches(p,f))}
    function emsStats(){const pts=state.frame.points||[];const good=pts.filter(p=>p.hasValue&&p.quality===1&&!p.stale).length;const writable=pts.filter(p=>p.writable).length;return{total:pts.length,good,bad:pts.length-good,writable}}
    function emsRows(kind){return emsVisiblePoints(kind).map(p=>{const s=status(p);return `<tr class="${esc(s.className)}"><td>${esc(p.index)}</td><td>${esc(p.deviceName||"")}</td><td>${esc(p.meterCode||"")}</td><td>${esc(p.pointCode||"")}</td><td class="name">${esc(p.name||"")}</td><td class="value ${esc(s.className)}">${esc(pointField(p,"value"))}</td><td class="muted">${esc(p.unit||"")}</td><td>${esc(p.quality??"")}</td><td class="muted">${esc(fmtTs(p.valueTs))}</td><td class="muted">${esc(p.writable?"是":"否")}</td></tr>`}).join("")}
    function renderEmsDashboard(screen){const stats=emsStats();const tabs=[["overview","首页"],["run","运行监测"],["pcs","PCS"],["bms","BMS"],["all","全部点位"]];const active=emsPageKind();const buttons=tabs.map(([k,label])=>`<button class="ems-tab ${active===k?"active":""}" data-ems-tab="${esc(k)}">${esc(label)}</button>`).join("");const rows=emsRows(active);const title=screen?.title||"KY EMS 本地显示";const css=emsDashboardCss;if(css!==lastRenderedCss){styleEl.textContent=css;lastRenderedCss=css}return `<main class="ems-shell"><header class="ems-header"><div><div class="ems-title">${esc(title)}</div><div class="ems-sub">machine: ${esc(state.frame.machineCode||"-")} &nbsp; points: ${esc(stats.total)} &nbsp; refresh: ${esc(displayRefreshIntervalMs)}ms</div></div><div><div class="ems-clock" data-ld-clock>${esc(new Date().toLocaleString())}</div><input class="ems-search" value="${esc(state.filter)}" placeholder="搜索 index / 设备 / 表计 / 点位 / 名称"></div></header><section class="ems-cards"><div class="ems-card"><div class="ems-card-title">配置点位</div><div class="ems-card-value">${esc(stats.total)}</div></div><div class="ems-card"><div class="ems-card-title">正常</div><div class="ems-card-value ok">${esc(stats.good)}</div></div><div class="ems-card"><div class="ems-card-title">异常/无值</div><div class="ems-card-value bad">${esc(stats.bad)}</div></div><div class="ems-card"><div class="ems-card-title">可写点</div><div class="ems-card-value accent">${esc(stats.writable)}</div></div></section><section class="ems-tabs"><div class="ems-tabbar">${buttons}</div><div class="ems-table-wrap"><table class="ems-table"><thead><tr><th>Index</th><th>设备</th><th>表计</th><th>点位编码</th><th>名称</th><th>值</th><th>单位</th><th>质量</th><th>时间</th><th>写</th></tr></thead><tbody>${rows}</tbody></table></div></section></main>`}
    ...
    function wireEmsDashboard(){const search=root.querySelector(".ems-search");if(search){search.addEventListener("input",ev=>{const cursor=ev.target.selectionStart||0;state.filter=ev.target.value||"";render(true);const next=root.querySelector(".ems-search");if(next){next.focus();try{next.setSelectionRange(cursor,cursor)}catch(e){}}})}root.querySelectorAll("[data-ems-tab]").forEach(btn=>btn.addEventListener("click",()=>{state.emsPage=btn.getAttribute("data-ems-tab")||"overview";render(true)}))}
    function screenUsesEmsDashboard(screen){return screen?.layout?.type==="emsDashboard"||screen?.type==="emsDashboard"||(screen?.widgets||[]).some(w=>w?.type==="emsDashboard")}
    function render(force){const signature=renderSignature()+"#"+(state.emsPage||"")+"#"+state.filter;if(!force&&signature===lastRenderSignature){refreshClockText();return}lastRenderSignature=signature;if(Array.isArray(displayScreens)&&displayScreens.length){const screen=activeScreen();root.innerHTML=screenUsesEmsDashboard(screen)?renderEmsDashboard(screen):renderComponents();wireEmsDashboard();return}const css=renderText(templateCss,false);if(css!==lastRenderedCss){styleEl.textContent=css;lastRenderedCss=css}root.innerHTML=renderText(templateHtml,true)}
