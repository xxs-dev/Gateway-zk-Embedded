...
### config/examples/device_dio_example.json

本机 DI/DO 使用独立 `local_dio` 协议，由 `DioDriver` 访问 Linux GPIO，不要配置成 `modbus_tcp`。只有该设备文件被 `deviceConfigFiles[]` 引用时才会启动 DIDO 服务。
...
舜通项目兼容导入会自动生成 `device_dio.json`，默认旧工程 `DI0-DI7` 映射到硬件 `DI6-DI13`；DO 按 `DO1-DO8` 映射。DI 和 DO 默认都按低有效处理，平台逻辑值统一为 `1=有效/闭合`、`0=无效/断开`。
