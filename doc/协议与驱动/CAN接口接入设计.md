...
驱动注册完设备点位后必须立即发布一次 `device_online=0`，不能等到首帧或首次超时才创建该点。在线状态还必须按 `onlineTimeoutMs` 与点位 TTL 的较小周期定时续期，不能只在在线/离线切换时写一次，否则值在长时间稳定运行后会过期。这样计算引擎能持续区分“已配置但尚未收到报文”和“该设备未配置”：前者参与全局故障并按离线处理，后者因路由不存在而跳过。
