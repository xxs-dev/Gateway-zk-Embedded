...
- 过期清理：按 app 级 `pointHistory.retentionDays` 分批删除 SQLite 历史，统一覆盖所有 `isStore=true` 点位
...
`runtime/apps/mqtt-service.json` 全局配置：
...
该期限由 Modbus、DLT645、DIO、CAN 和 IEC 驱动统一读取。旧 app 配置未提供时默认 30 天；不在设备或点位层重复配置。
...
- SQLite 清理每批最多删除 100000 行，避免既有大库产生超大事务；清理后的空闲页由后续写入复用，不自动执行会长时间锁库的 `VACUUM`
...
- 2026-06-08 已验证版本将点位注册改为批量注册，并优化 `removeExpired()`：先构建活跃 owner 和活跃 claim 集合，再扫描 latest，避免在共享内存锁内执行 latest x claim 的重复遍历。该版本已在 `192.168.22.16` 和 `10.126.126.2` 通过生产烟测。
- 生产升级后如果保留了旧版本共享内存段，可能遗留旧 mutex 或旧 ring 状态。保守流程是先停 `gateway-services.service`，清理 `/dev/shm/gateway_point_store*`，再启动新版本。
- `pointUpdates` 是实时变更队列。若长期接近 `65535/65535`，说明消费端未及时 drain；2026-06-08 生产验证中，`10.126.126.2` 升级前队列为满，升级并重建共享内存后最终观察为 `0/65535`。
