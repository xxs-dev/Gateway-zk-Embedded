...
- `monitor-service.json:localDisplay.enabled=true` 且 `localDisplay.renderer=webkit` 或未配置：`local-display@monitor-service`
- WebKit 模式下 `localDisplay.kiosk.enabled=true` 且检测到显示输出：`local-kiosk@monitor-service`
