...
- `PointStoreRouter` 负责建立 `(sharedMemoryName, index) -> meterCode / pointCode / writable` 精确路由，并保留仅适用于无歧义场景的 `index` 兼容查询。
...
- pointctl 要支持多 shm。
- MQTT 要聚合多 shm。
- 同一 shm 内必须校验 index 唯一；跨 shm 同号允许兼容老项目，但上层必须携带 shm 精确定位。
- Java 配置生成要知道接口和 shm 的对应关系。

### 点位 index 规划

为了便于平台配置和人工排障，新项目仍建议规划为全网关唯一 `index`；运行时的硬约束是同一 `sharedMemoryName` 内唯一。老项目在不同共享内存复用同一 `index` 时，通过复合路由正常运行。
...
- `(sharedMemoryName, index)` 必须全网关唯一；跨共享内存的同号 `index` 允许存在。
...
- 新项目建议保持 `index` 全网关唯一，兼容项目至少保证同一共享内存内唯一。
- 写命令必须按 `sharedMemoryName + index` 精确路由；只带 `index` 的兼容写入遇到歧义必须拒绝。
...
- C++ 已增加 `PointStoreRouter`，负责维护 `(sharedMemoryName, index) -> machineCode / meterCode / pointCode / interface` 精确路由，并为无歧义旧调用保留主路由。
...
- 如果多个设备配置在同一个 `sharedMemoryName` 内重复使用同一 `index`，Router 启动阶段会直接拒绝；不同共享内存同号允许加载，SCADA 单点读写使用复合键，无法证明唯一目标的批量写入直接拒绝。
...
for each sharedMemoryName:
  ...
  for each update:
    route = PointStoreRouter.routeByLocation(sharedMemoryName, update.index)
    latest = router.getLatestByLocation(sharedMemoryName, update.index)
    evaluate alarm
