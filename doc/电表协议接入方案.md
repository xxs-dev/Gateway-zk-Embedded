...
DLT645 使用独立 `Dlt645CommandExecutor`，写入仍复用共享内存 pending queue、`pointctl write` 和 MQTT 命令回执。采集与写入共享同一个客户端事务锁，避免半双工串口帧交叉。
...
当前支持 `dlt645_scheduled_control`、`dlt645_none`、`dlt645_bcd`、`dlt645_uint_le` 和 `dlt645_fixed_hex`。协议 `94H` 只表示设备接受命令，不能替代物理闸位反馈。
